...
class SearchHorseUseCaseImpl(
    ...
    override fun execute(input: SearchHorseUseCase.SearchHorseQuery): PageResult<HorseResult> {
        val horses = searchHorse.search(input, SearchOutputPort.PageRequest(input.page, input.size))
        return HexagonalMapper.mapToResult(horses, HorseMapper::mapToResult)
    }
}
